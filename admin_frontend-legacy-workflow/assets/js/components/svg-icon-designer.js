/**
 * SVG Icon Designer Component
 * Handles SVG upload, validation, styling, and preview for marker categories
 */
...
const logger = new DebugLogger('SVGIconDesigner');

class SVGIconDesigner {
    constructor(containerId, options = {}) {
        ...
        this.currentConfig = {
            ...
            svgContent: null,
            style: {
                color: '#2563eb', // Use admin frontend primary color
                backgroundColor: '#ffffff',
                borderColor: '#e2e8f0', // Use admin frontend border color
                borderWidth: 2,
    ...
    /**
     * Render the SVG icon designer interface
     */
    render() {
        ...
        container.innerHTML = `
            <div class="svg-icon-designer">
                <div class="designer-header">
                    <div class="header-content">
                        <div class="header-text">
                            <h3>SVG Icon Designer</h3>
                            <p>Upload and customize an SVG icon for this marker category</p>
                        </div>
                        <div class="icon-preview-corner" id="icon-preview-corner">
                            <div class="corner-icon" id="corner-icon">
                    ...
                    <div class="design-section">
                        ...
                        <div class="input-method-toggle">
                            <label class="toggle-option">
                                <input type="radio" name="input-method" value="upload" checked>
                                <span>Upload File</span>
                            </label>
                            <label class="toggle-option">
                                <input type="radio" name="input-method" value="paste">
                        ...
                        <div class="input-section" id="upload-section">
                            <div class="svg-upload-area" id="svg-upload-area">
                        ...
                        <!-- Code Input Section -->
                        <div class="input-section" id="paste-section" style="display: none;">
                            <div class="svg-code-input">
                                <textarea id="svg-code-input" placeholder="Paste your SVG code here...
Example:
<svg width='24' height='24' viewBox='0 0 24 24' fill='none' xmlns='http://www.w3.org/2000/svg'>
  <path d='M12 2L2 7V10C2 16 6 20.5 12 22C18 20.5 22 16 22 10V7L12 2Z' stroke='currentColor' stroke-width='2' stroke-linejoin='round'/>
                                ...
                                <div class="code-actions">
                                    <button type="button" class="btn btn-sm btn-secondary" id="clear-code-btn">Clear</button>
                                    <button type="button" class="btn btn-sm btn-primary" id="apply-code-btn">Apply SVG</button>
                                </div>
                            </div>
                        </div>
                        
                        <div class="svg-info" id="svg-info" style="display: none;"></div>
                    </div>
                            ...
                            <div class="accordion-content">
                                <div class="style-controls">
                                    <div class="control-row">
                                        ...
                                        <input type="color" id="icon-color" value="${this.currentConfig.style.color}">
                                    </div>
                                    <div class="control-row">
                                        <label>Background Color:</label>
                                        <input type="color" id="bg-color" value="${this.currentConfig.style.backgroundColor}">
                                    </div>
                                    <div class="control-row">
                                        <label>Border Color:</label>
                                        <input type="color" id="border-color" value="${this.currentConfig.style.borderColor}">
                                    </div>
                                    <div class="control-row">
                                        <label>Border Width:</label>
                                        <input type="range" id="border-width" min="0" max="5" value="${this.currentConfig.style.borderWidth}">
                                        <span id="border-width-value">${this.currentConfig.style.borderWidth}px</span>
                                    </div>
                                    <div class="control-row">
                                        <label>Size:</label>
                                        <select id="icon-size">
                                    ...
                                    <div class="control-row">
                                        <label>
                                            <input type="checkbox" id="icon-shadow" ${this.currentConfig.style.shadow ? 'checked' : ''}>
                                            Drop Shadow
                                        </label>
                                    </div>
                                </div>
                ...
                <div class="designer-footer">
                    ...
                    <button class="btn btn-primary" id="save-btn" disabled>Save Icon</button>
                </div>
            </div>
        `;
    ...
    setupEventListeners() {
        ...
        // SVG Code Input
        const applyCodeBtn = container.querySelector('#apply-code-btn');
        const clearCodeBtn = container.querySelector('#clear-code-btn');
        const svgCodeInput = container.querySelector('#svg-code-input');
        ...
        // Style Controls
        container.querySelector('#icon-color').addEventListener('input', (e) => {
            this.currentConfig.style.color = e.target.value;
            this.updatePreview();
        });
        
        container.querySelector('#bg-color').addEventListener('input', (e) => {
            this.currentConfig.style.backgroundColor = e.target.value;
            this.updatePreview();
        });
        
        container.querySelector('#border-color').addEventListener('input', (e) => {
            this.currentConfig.style.borderColor = e.target.value;
            this.updatePreview();
        });
        
        container.querySelector('#border-width').addEventListener('input', (e) => {
            this.currentConfig.style.borderWidth = parseInt(e.target.value);
            container.querySelector('#border-width-value').textContent = `${e.target.value}px`;
            this.updatePreview();
        });
        
        container.querySelector('#icon-size').addEventListener('change', (e) => {
            this.currentConfig.style.size = parseInt(e.target.value);
            this.updatePreview();
        });
        
        container.querySelector('#icon-shape').addEventListener('change', (e) => {
            this.currentConfig.style.shape = e.target.value;
            this.updatePreview();
        });
        
        container.querySelector('#icon-shadow').addEventListener('change', (e) => {
    ...
    async handleCodeInput(showMessage = true) {
        const svgCodeInput = document.querySelector('#svg-code-input');
        ...
        if (!svgContent) {
            ...
            // Clear content if textarea is empty
            this.clearSVGContent();
            return;
        }
        
        logger.log('Handling SVG code input');
        ...
        try {
            ...
            // Store SVG content and metadata
            ...
            logger.error('Error processing SVG code:', error);
            if (showMessage) {
     ...
     */
    clearPreviousState() {
        // Clear the corner preview to ensure clean state
        const cornerIcon = document.querySelector('#corner-icon');
        
        if (cornerIcon) {
            cornerIcon.innerHTML = '<span class="placeholder-icon">📍</span>';
            cornerIcon.style.cssText = '';
        }
        
        // Force a brief delay to ensure DOM updates
        setTimeout(() => {
            this.updatePreview();
        }, 10);
    }
    ...
    clearSVGContent() {
        ...
        // Clear info display
        const infoDiv = document.querySelector('#svg-info');
        if (infoDiv) {
            infoDiv.style.display = 'none';
        }
        
        // No SVG code display to clear
    ...
    async handleFileUpload(file) {
        logger.log('Handling SVG file upload:', file.name);
        ...
        try {
            ...
            logger.error('Error processing SVG file:', error);
    ...
    updateCornerPreview() {
        ...
        if (this.currentConfig.svgContent) {
            // Use uploaded/pasted SVG - ensure we get fresh content
            iconHTML = this.currentConfig.svgContent.trim();
        } else {
            // Use placeholder
            iconHTML = '<span class="placeholder-icon">📍</span>';
        }
        
        // Clear existing content first to prevent caching issues
        ...
        // Add data attribute for shape-specific CSS targeting
        cornerIcon.setAttribute('data-shape', style.shape);
        
        // Apply styling (fixed size for corner preview)
     ...
     */
    generateIconCSS(style, context = 'marker') {
        let css = `
            width: ${style.size}px;
    ...
    /**
     * Save icon configuration to database
     */
    async saveIconConfiguration() {
        if (!this.options.categoryId || !this.currentConfig.svgContent) {
            alert('Missing category ID or SVG content.');
            return;
        }
        
        try {
            const projectId = this.options.projectId || supabaseClient.getCurrentProjectId();
            
            const updateData = {
                icon_config: this.currentConfig
            };
            
            await supabaseClient.updateWithProjectContext(
                'marker_categories', 
                projectId, 
            ...
            alert('Icon configuration saved successfully!');
            
        } catch (error) {
            logger.error('Error saving icon configuration:', error);
            alert('Failed to save icon configuration. Please try again.');
        }
    }
    
    /**
     * Load existing icon configuration
     */
    async loadConfiguration(categoryId) {
        try {
            const projectId = this.options.projectId || supabaseClient.getCurrentProjectId();
            ...
            if (data && data.icon_config && Object.keys(data.icon_config).length > 0) {
                this.currentConfig = { ...this.currentConfig, ...data.icon_config };
                this.updateUIFromConfig();
                this.updatePreview();
            }
            
        } catch (error) {
            logger.error('Error loading icon configuration:', error);
        }
    }
    ...
    updateUIFromConfig() {
        ...
        container.querySelector('#icon-color').value = style.color;
        container.querySelector('#bg-color').value = style.backgroundColor;
        container.querySelector('#border-color').value = style.borderColor;
        container.querySelector('#border-width').value = style.borderWidth;
        container.querySelector('#border-width-value').textContent = `${style.borderWidth}px`;
        container.querySelector('#icon-size').value = style.size;
        container.querySelector('#icon-shape').value = style.shape;
        container.querySelector('#icon-shadow').checked = style.shadow;
        
        if (this.currentConfig.svgContent) {
            // Populate the code input textarea
            const svgCodeInput = container.querySelector('#svg-code-input');
            if (svgCodeInput) {
                svgCodeInput.value = this.currentConfig.svgContent;
            }
            
            container.querySelector('#save-btn').disabled = false;
...
export default SVGIconDesigner;