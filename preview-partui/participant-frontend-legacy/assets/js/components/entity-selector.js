...
// Note: app import removed - notifications will be handled by form renderer
const app = null; // Placeholder to prevent errors
...
class EntitySelector {
    constructor(containerId, options = {}) {
        ...
        this.options = {
            ...
            // Local data option (bypasses database queries)
        ...
        this.logger = new DebugLogger('EntitySelector');
     ...
     */
    async loadEntities() {
        // If local data is provided, use it instead of database query
        if (this.options.localData) {
            this.logger.log('EntitySelector using local data for', this.options.tableName || 'dummy:', this.options.localData);
            this.entities = this.options.localData;
            return;
        }

        try {
            let query = supabaseClient.getParticipantClient()
            ...
            this.logger.error(`Failed to load ${this.options.entityNamePlural}:`, error);
            this.entities = [];
    ...
    render() {
        ...
        if (!container) {
            this.logger.error(`Container ${this.containerId} not found`);
            return;
        }
    ...
    async createAndSelectEntity(entityName) {
        ...
        try {
            ...
            this.logger.error(`Failed to create ${this.options.entityName}:`, error);
            if (this.options.onError) {
                this.options.onError(error);
            }
            if (app && app.showNotification) {
    ...
    /**
     * Refresh entities from database or local data
     */
    async refresh() {
        await this.loadEntities();
        this.hideDropdowns();
    ...
    /**
     * Update local data and refresh entities
     */
    updateLocalData(newData) {
        if (this.options.localData) {
            this.options.localData = newData;
