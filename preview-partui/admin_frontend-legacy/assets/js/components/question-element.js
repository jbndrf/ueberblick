...
const logger = new DebugLogger('QuestionElement');
...
class QuestionElement {
    ...
    renderPreview() {
        switch (this.fieldType) {
            ...
            case 'smart_dropdown':
                const options = this.data.dynamicOptions || [];
                const sourceField = this.data.field_options?.source_field;
                const sourceTable = this.data.field_options?.source_table_id;
                ...
                if (options.length > 0) {
                    ...
                    const sourceInfo = sourceField ? `field: ${sourceField}` : 
                                     sourceTable ? `custom table (${sourceTable})` : 
                                     'Not configured';
                    return `
                        <select disabled>
                            <option>Dynamic options from ${sourceInfo}</option>
                        </select>
                    `;
                }
