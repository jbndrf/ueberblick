...
<div class="step-config-panel">
	...
	<div class="config-header">
		<button class="back-btn" onclick={onBack}>
			...
			<span>{automationStepConfigPanelSteps?.() ?? 'Steps'}</span>
		</button>
		<span class="step-label">{automationStepConfigPanelStep?.() ?? 'Step'} {stepIndex + 1}</span>
	</div>
	...
	<div class="config-content">
		...
		<div class="config-section">
			<Input
				...
				placeholder={automationStepConfigPanelStepNamePlaceholder?.() ?? 'Step name...'}
				class="h-7 text-xs"
			/>
		</div>
		...
		<!-- Actions -->
		<div class="config-section">
			<span class="section-label">{automationStepConfigPanelActionsLabel?.() ?? 'Actions'}</span>
			<ActionBuilder
