<script lang="ts">
	...
	import {
		propertiesButtonConfigPopoverColorField,
		propertiesButtonConfigPopoverConfirmationDefault,
		propertiesButtonConfigPopoverConfirmationDescription,
		propertiesButtonConfigPopoverConfirmationMessageField,
		propertiesButtonConfigPopoverLabelField,
		propertiesButtonConfigPopoverRequiresConfirmation,
		propertiesButtonConfigPopoverTitle
	...
	const confirmationMessage = $derived(config.confirmation_message || (propertiesButtonConfigPopoverConfirmationDefault?.() ?? 'Are you sure you want to proceed?'));
	...
	function handleConfirmationToggle(checked: boolean) {
		updateConfig({
			...
			confirmation_message: checked ? (config.confirmation_message || (propertiesButtonConfigPopoverConfirmationDefault?.() ?? 'Are you sure you want to proceed?')) : undefined
		});
	}
...
<Popover.Root bind:open={popoverOpen}>
	...
	<Popover.Content class="button-config-popover" align="end" sideOffset={4}>
		<div class="popover-header">
			<span class="popover-title">{propertiesButtonConfigPopoverTitle?.() ?? 'Button Configuration'}</span>
		</div>

		<div class="popover-content">
			<div class="form-field">
				<Label for="btn-label">{propertiesButtonConfigPopoverLabelField?.() ?? 'Button Label'}</Label>
				<Input
					id="btn-label"
					value={buttonLabel}
			...
			<div class="form-field">
				<Label for="btn-color">{propertiesButtonConfigPopoverColorField?.() ?? 'Button Color'}</Label>
			...
			<div class="form-field-switch">
				<div class="switch-info">
					<Label for="btn-confirm">{propertiesButtonConfigPopoverRequiresConfirmation?.() ?? 'Requires Confirmation'}</Label>
					<p class="switch-description">
						{propertiesButtonConfigPopoverConfirmationDescription?.() ?? 'Show a confirmation dialog'}
					</p>
				</div>
				<Switch
					id="btn-confirm"
			...
			{#if requiresConfirmation}
				<div class="form-field">
					<Label for="btn-confirm-msg">{propertiesButtonConfigPopoverConfirmationMessageField?.() ?? 'Confirmation Message'}</Label>
					<Input
						id="btn-confirm-msg"
						value={confirmationMessage}
						oninput={handleConfirmationMessageChange}
						placeholder={propertiesButtonConfigPopoverConfirmationDefault?.() ?? 'Are you sure you want to proceed?'}
					/>
				</div>
			{/if}
		</div>
