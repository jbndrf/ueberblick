<script lang="ts">
	...
	import { workflowBuilderActionPanelEditAction, workflowBuilderActionPanelTransitionAction, workflowBuilderActionPanelUnnamedAction } from '$lib/paraglide/messages';
	...
	const actionLabel = $derived((context.action.label as string) || (workflowBuilderActionPanelUnnamedAction?.() ?? 'Unnamed Action'));
</script>
...
<div class="panel">
	<div class="panel-header bg-muted border-b border-border">
		...
		<div class="panel-header-text">
			...
			<span class="panel-header-subtitle text-muted-foreground">{isEditAction ? (workflowBuilderActionPanelEditAction?.() ?? 'Edit action') : (workflowBuilderActionPanelTransitionAction?.() ?? 'Transition action')}</span>
		</div>
	</div>
