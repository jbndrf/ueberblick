<script lang="ts">
	...
	import {
		...
		editToolEditorViewLocationPreviewTitle,
	...
	type Props = {
		...
		/** Callback when editable fields change (field_def ids) */
	...
	// `editable_fields` references workflow_field_defs ids. The ancestor groups
	// carry ref rows whose `.id` is the ref id — remap each field's identity to
	...
	const selectedFieldsWithMeta = $derived.by(() => {
		...
		for (const fieldId of selectedFieldIds) {
			for (const group of defScopedFields) {
	...
	<div class="editor-content">
		...
		<div class="left-panel">
			<div class="mode-header">
				<span class="mode-label">{editToolEditorViewEditMode?.() ?? 'Edit Mode'}</span>
				<DropdownMenu.Root>
					<DropdownMenu.Trigger class="mode-dropdown-trigger">
						{#if editTool.edit_mode === 'form_fields'}
							<span>{editToolEditorViewModeFormFields?.() ?? 'Form Fields'}</span>
						{:else}
							<span>{editToolEditorViewModeLocation?.() ?? 'Location'}</span>
						{/if}
					...
					<DropdownMenu.Content align="end">
						...
						>
							{editToolEditorViewModeFormFields?.() ?? 'Form Fields'}
						</DropdownMenu.Item>
						<DropdownMenu.Item
							onclick={() => onEditModeChange?.('location')}
							class={editTool.edit_mode === 'location' ? 'bg-accent' : ''}
						>
							{editToolEditorViewModeLocation?.() ?? 'Location'}
						</DropdownMenu.Item>
			...
			{#if editTool.edit_mode === 'form_fields'}
				<AncestorFieldsPanel
					ancestorFields={defScopedFields}
					{selectedFieldIds}
