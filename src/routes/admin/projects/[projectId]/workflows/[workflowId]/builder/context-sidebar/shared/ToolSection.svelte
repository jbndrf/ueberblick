<script lang="ts">
	...
	import { workflowBuilderToolSectionTitle } from '$lib/paraglide/messages';
...
<div class="tool-section border-b border-border">
	<button class="tool-section-header bg-muted hover:bg-accent border-b border-border" onclick={() => (isOpen = !isOpen)}>
		<span class="tool-section-title text-muted-foreground">{workflowBuilderToolSectionTitle?.() ?? title}</span>
