<script lang="ts">
	...
	import { participantLayerSheetBaseLayers, participantLayerSheetDescription, participantLayerSheetNoLayersConfigured, participantLayerSheetOverlayLayers, participantLayerSheetTitle } from '$lib/paraglide/messages';
...
<Sheet.Root bind:open>
	<Sheet.ContentNoOverlay side="left" class="w-80">
		<Sheet.Header>
			<Sheet.Title>{participantLayerSheetTitle?.() ?? 'Map Layers'}</Sheet.Title>
			<Sheet.Description>{participantLayerSheetDescription?.() ?? 'Select base map and overlay layers'}</Sheet.Description>
		</Sheet.Header>
		...
		<div class="space-y-6 py-6">
			...
			{#if baseLayers.length > 0}
				<div>
					<h4 class="mb-3 text-sm font-medium">{participantLayerSheetBaseLayers?.() ?? 'Base Layers'}</h4>
			...
			{#if overlayLayers.length > 0}
				<div>
					<h4 class="mb-3 text-sm font-medium">{participantLayerSheetOverlayLayers?.() ?? 'Overlay Layers'}</h4>
					<div class="space-y-2">
			...
			{#if baseLayers.length === 0 && overlayLayers.length === 0}
				<div class="py-8 text-center text-sm text-muted-foreground">
					{participantLayerSheetNoLayersConfigured?.() ?? 'No map layers configured for this project'}
				</div>
			{/if}
		</div>
