<script lang="ts">
	...
	const title = $derived(
		context.attachedTo.type === 'global' ? 'Global Tools' :
		context.attachedTo.type === 'stage' ? 'Stage Tools' : 'Connection Tools'
	);

	const subtitle = $derived(
		context.attachedTo.type === 'global' ? 'Available on all stages' : 'Select a tool to add'
	);
</script>
