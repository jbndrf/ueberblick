...
/**
 * Tile download progress store
 */
const tileProgressStore = writable<TileDownloadProgress | null>(null);
export const tileProgress: Readable<TileDownloadProgress | null> = {
	subscribe: tileProgressStore.subscribe
};
...
export function updateTileProgress(downloadedTiles: number): void {
	tileProgressStore.update((current) => {
