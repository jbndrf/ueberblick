<script lang="ts" generics="TData">
	...
	import {
		adminBaseTableAutoGenerated,
		adminBaseTableEditModeDefault,
		adminBaseTableEmptyDefault,
		adminBaseTableOpenMenu,
		adminBaseTableSelectAll,
		adminBaseTableSelectRow,
		commonDelete,
		commonEdit,
		rolesActions
	...
	let {
		...
		createAreaPrefix,
		inlineRowCreation,
		emptyMessage = (adminBaseTableEmptyDefault?.() ?? 'No data found'),
		emptySubMessage,
		showToolbar = true,
		showEditMode = false,
		editModeLabel = (adminBaseTableEditModeDefault?.() ?? 'Edit mode')
	}: Props = $props();
	...
	const tableColumns = $derived.by((): InternalColumnDef<TData>[] => {
		...
		if (rowActions && (rowActions.onEdit || rowActions.onDelete || rowActions.customActions)) {
			cols.push({
				id: 'actions',
				header: () => rowActions.header || rolesActions(),
...
{#snippet selectAllSnippet({ table }: { table: any })}
	<Checkbox
		...
		aria-label={adminBaseTableSelectAll?.() ?? 'Select all'}
	/>
{/snippet}
...
{#snippet selectSnippet({ row }: { row: any })}
	<Checkbox
		...
		aria-label={adminBaseTableSelectRow?.() ?? 'Select row'}
	/>
{/snippet}
		...
		<DropdownMenu.Trigger>
			{#snippet child({ props })}
				<Button {...props} variant="ghost" size="icon" class="h-8 w-8">
					...
					<span class="sr-only">{adminBaseTableOpenMenu?.() ?? 'Open menu'}</span>
				</Button>
			{/snippet}
		</DropdownMenu.Trigger>
						...
						<Table.Row class="group {hasNewRowInput ? 'bg-accent/10' : 'bg-muted/20'}">
							{#each tableColumns as column}
								...
								>
									...
									{:else if isExcluded}
										<span class="text-muted-foreground text-xs">{adminBaseTableAutoGenerated?.() ?? 'auto'}</span>
									{:else}
