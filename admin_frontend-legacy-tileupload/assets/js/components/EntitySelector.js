...
const logger = new DebugLogger('EntitySelector');
...
class EntitySelector {
    constructor(containerId, options = {}) {
        ...
        this.options = {
            ...
            // Local data - if provided, skip database loading
            localData: options.localData || null,
            
            // Initial selection - entities to be selected when component loads
    ...
    async init() {
        ...
        // Process initial selection if provided
        if (this.options.initialSelection && this.options.initialSelection.length > 0) {
            await this.setSelectedEntities(this.options.initialSelection);
        }
    ...
    /**
     * Load entities from database or local data
     */
    async loadEntities() {
        try {
            // Use local data if provided (for local-first architecture)
            if (this.options.localData) {
                logger.log(`EntitySelector using local data for ${this.options.tableName}:`, this.options.localData);
                this.entities = this.options.localData;
                return;
            }
            ...
            logger.error(`EntitySelector: Failed to load ${this.options.entityNamePlural}:`, error);
            this.entities = [];
    ...
    render() {
        ...
        if (!container) {
            logger.error(`EntitySelector: Container ${this.containerId} not found`);
            return;
        }
    ...
    async createAndSelectEntity(entityName) {
        ...
        try {
            ...
            logger.error(`EntitySelector: Failed to create ${this.options.entityName}:`, error);
            if (this.options.onError) {
                this.options.onError(error);
            }
            if (app && app.showNotification) {
    ...
    selectEntity(entityId, entityName) {
        ...
        logger.log('EntitySelector: Entity selected, calling onSelectionChange with:', this.selectedEntities);
        ...
        if (this.options.onSelectionChange) {
            logger.log('EntitySelector: Calling onSelectionChange callback');
            this.options.onSelectionChange(this.selectedEntities);
        } else {
            logger.warn('EntitySelector: No onSelectionChange callback configured');
        }
    }
    ...
    removeEntity(entityId) {
        ...
        logger.log('EntitySelector: Entity removed, calling onSelectionChange with:', this.selectedEntities);
        
        if (this.options.onSelectionChange) {
            logger.log('EntitySelector: Calling onSelectionChange callback for removal');
            this.options.onSelectionChange(this.selectedEntities);
        } else {
            logger.warn('EntitySelector: No onSelectionChange callback configured for removal');
        }
    }
